...
def test_naive_forecaster_fill_empty_data():
...
def test_naive_forecaster_predict():
...
def test_naive_forecaster_predict_without_fit():
    """Test that predict() raises RuntimeError if called before fit()"""
    model = NaiveForecaster()

    with pytest.raises(RuntimeError, match="Model must be fitting before calling predict"):
        model.predict(5)

def test_naive_forecaster_implements_base_interface():
...
def test_naive_forecaster_with_single_data_point():
...
def test_naive_forecaster_with_irregular_data():
